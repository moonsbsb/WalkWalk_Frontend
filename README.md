...
개인 프로젝트 - 기획, 디자인, Android, Backend 담당  
...
- 산책 시작, 일시정지, 종료 상태 관리  
- 산책 완료 후 속력을 기반으로 산책 유형 및 산책 데이터 제공

### 산책 패턴 분석
- 위치 이동 속도(location.time)기반으로 산책 유형 분석
...
- collect()를 통해 UI에 최신 데이터 반영  

### 다중 Retrofit 인스턴스 분리
- 서버 API + 기상청 API 동시 사용으로 Retrofit 충돌 문제 발생
- @MainRetrofit, @WeatherRetrofit으로 Qualifier 정의를 통해 해결  
  

## 트러블 슈팅  

1. 지도 초기화 지연으로 인한 크래시
...
**해결**
- kakaoMap 초기화 전까지 위치 데이터 수집하지 않는 조건문 추가
...
- 클라이언트: 변수 loading을 통해 비동기 처리 상태 관리하고 상태를 참고해서 UI에서 버튼 비활성화  
